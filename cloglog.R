...
# Generate random values for beta
set.seed(42)
...
pr_1 <- inverse_cloglog(X = X_1, beta = beta)
...
# n = 1000
X_2 <- matrix(rnorm(1000*P), 1000, P)
pr_2 <- inverse_cloglog(X = X_2, beta = beta)
y_2 <- rbinom(1000, 1, pr_2)

data_list_2 <- list(X = X_2, N = 1000, y = y_2, P = P)
logit_fit_2 <- stan(model_code = logit_model, data = data_list_2, cores = 1, chains = 2, iter = 2000)
probit_fit_2 <- stan(model_code = probit_model, data = data_list_2, cores = 1, chains = 2, iter = 2000, save_warmup = FALSE)
cloglog_fit_2 <- stan(model_code = cloglog_model, data = data_list_2, cores = 1, chains = 2, iter = 2000, init_r = 0.1, save_warmup = FALSE)
gev_fit_2 <- stan(model_code = gev_model, data = data_list_2, cores = 1, chains = 2, iter = 2000, init_r = 0.1)

# n = 5000
X_3 <- matrix(rnorm(5000*P), 5000, P)
pr_3 <- inverse_cloglog(X = X_3, beta = beta)
...
print(logit_fit_3, pars = "beta") # beta[1] is the constant
print(probit_fit_3, pars = "beta")
print(cloglog_fit_3, pars = "beta") # beta[1] is the constant
print(gev_fit_3, pars = "beta")
...
known_parameter <- data_frame(variable = paste0("beta[",1:(P+1),"]"), real_value = beta)
# extract params as a (draws * number of chains * number of params) array
extract(gev_fit, permute = F, pars = "beta") %>%
  # stack the chains on top of one another and drop the chains label
  plyr::adply(2) %>%
  dplyr::select(-chains) %>%
  # converting from wide form to long form
...
llik_logit <- extract_log_lik(logit_fit_3, parameter_name = "log_lik")
...
llik_probit <- extract_log_lik(probit_fit_3, parameter_name = "log_lik")
...
llik_cloglog <- extract_log_lik(cloglog_fit_3, parameter_name = "log_lik")
...
loo_gev <- loo(llik_gev, k_threshold = 0.7)
print("GEV model")
print(loo_gev)
...
# Extract params as a (draws * number of chains * number of params) array
plot_data <- extract(probit_fit_3, permuted = F, pars = c("pr")) %>%
  plyr::adply(2) %>% 
  dplyr::select(-chains) %>% 
  # Stack the chains on top of one another and drop the chains label
  melt() %>% 
